...
function parse_yaoscript(ex)
    @match ex begin
        ...
        _ =>
            error("wrong format, expect expression like `let nqubits=5 GATEDEF end`, got $ex")
    end
end
    ...
    @match ex begin
        ...
        :($exloc => Measure($op) => $post) => begin
            ...
            op = op isa Nothing || op == :nothing ? :(ComputationalBasis()) :
                    parse_ex(
                op,
                exloc == :ALL ? info : ParseInfo(length(locs), info.version),
            )
            @match post begin
                ::Nothing || :nothing =>
