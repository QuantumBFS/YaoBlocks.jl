...
struct CachedBlock{ST, BT, N, T} <: TagBlock{N, T}
    ...
    function CachedBlock(server::ST, x::BT, ::HasMatrix, level::Int) where {ST, N, T, BT <: AbstractBlock{N, T}}
        alloc!(server, x, CacheFragment(x))
        new{ST, BT, N, T}(server, x, level)
    end
end

CachedBlock(s, x, level) = CachedBlock(s, x, MatrixTrait(x), level)

CacheServers.iscached(c::CachedBlock) = iscached(c.server, c.block)
iscacheable(c::CachedBlock) = iscacheable(c.server, c.block)
chcontained_block(cb::CachedBlock, blk::AbstractBlock) =
    CachedBlock(cb.server, blk, cb.level)
